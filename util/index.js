...
function Session(options) {
    this.retries =
        options && options.retries != undefined ? options.retries : 1;
    ...
    if (this.packetSize < 12) this.packetSize = 12;
    ...
    this.sessionId =
        options && options.sessionId ? options.sessionId : process.pid;
...
Session.prototype.getSocket = function () {
    ...
    var protocol =
        this.addressFamily == raw.AddressFamily.IPv6
            ? raw.Protocol.ICMPv6
            : raw.Protocol.ICMP;
...
Session.prototype.fromBuffer = function (buffer) {
    ...
    if (this.addressFamily == raw.AddressFamily.IPv6) {
        ...
        if (buffer.length < 20 || (buffer[0] & 0xf0) != 0x40) return;
        ...
        if (buffer.length - ip_length < 8) return;

        var ip_icmp_offset = ip_length;

        // ICMP message too short
        if (buffer.length - ip_icmp_offset < 8) return;
        ...
        if (type == 3 || type == 4 || type == 5 || type == 11) {
            ...
            // Need at least 20 bytes for an IP header, and it should be IPv4
            if (
                buffer.length - ip_icmp_ip_offset < 20 ||
    ...
    if (buffer.readUInt16BE(offset + 4) != this.sessionId) return;
...
Session.prototype.onSocketMessage = function (buffer, source) {
    if (this._debug) this._debugResponse(source, buffer);
    ...
    if (req) {
         ...
         **/
        if (this.addressFamily == raw.AddressFamily.IPv6) {
            if (req.type == 128) return;
        } else {
            if (req.type == 8) return;
        }
        ...
        if (this.addressFamily == raw.AddressFamily.IPv6) {
            if (req.type == 1) {
                req.callback(
                    new DestinationUnreachableError(source),
                    req.target,
                    req.sent,
                    new Date()
                );
                ...
                req.callback(
                    new ParameterProblemError(source),
                    req.target,
                    req.sent,
                    new Date()
                );
            } else if (req.type == 129) {
                req.callback(null, req.target, req.sent, new Date());
            } else {
                req.callback(
                    new Error(
            ...
            if (req.type == 0) {
                ...
                req.callback(
                    new RedirectReceivedError(source),
                    req.target,
                    req.sent,
                    new Date()
                );
...
Session.prototype.onTimeout = function (req) {
    if (req.retries > 0) {
        ...
        this.reqRemove(req.id);
        req.callback(
            new RequestTimedOutError("Request timed out"),
            req.target,
            req.sent,
            new Date()
        );
...
Session.prototype._generateId = function () {
    ...
    while (1) {
        ...
        // No free request IDs
        if (this.nextId == startId) return;
    }
};

Session.prototype.pingHost = function (target, lost, callback) {
...
Session.prototype.reqRemove = function (id) {
    ...
    if (this.reqsPending <= 0)
        if (!this.getSocket().recvPaused) this.getSocket().pauseRecv();
    return req;
};
...
Session.prototype.toBuffer = function (req) {
    ...
    const b1 = Buffer.from(JSON.stringify(req.lost.slice(0, 10)));
    console.log(`buffer:${b1},length:${b1.length}`);
...
Session.prototype.traceRouteCallback = function (
    trace,
    req,
    error,
    target,
    ...
    if (error) {
        ...
        if (
            ...
            ++trace.timeouts >= trace.maxHopTimeouts
        ) {
...
Session.prototype.traceRoute = function (
    target,
    ttlOrOptions,
    feedCallback,
    doneCallback
    ...
    if (typeof ttlOrOptions == "object") {
        if (ttlOrOptions.ttl) ttl = ttlOrOptions.ttl;
        if (ttlOrOptions.maxHopTimeouts)
            maxHopTimeouts = ttlOrOptions.maxHopTimeouts;
        if (ttlOrOptions.startTtl) startTtl = ttlOrOptions.startTtl;
    } else {
        ttl = ttlOrOptions;
    }
